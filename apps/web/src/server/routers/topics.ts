...
export const topicsRouter = router({
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Invalidate trending and search caches when new topics are created
      await invalidateCache("topics.trending:*");
      await invalidateCache("topics.search:*");
      await invalidateCache("topics.byCategory:*");
  ...
  trending: publicProcedure
    ...
    .query(async ({ ctx: _ctx, input }) => {
      const result = await getCached(
        "topics.trending",
        input,
        30, // 30s TTL — trending changes frequently
        async () => {
